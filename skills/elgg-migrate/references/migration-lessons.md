...
| **Removed instance method still called** | `ElggPlugin::getUserSetting()`, `…::getManifest()`, etc. | varies (manifest/getUserSetting removed 4.0/5.x) | the `elgg_get_plugin_user_setting()` / `getDisplayName()` equivalent | route crawl + `Call to undefined method` log check |
