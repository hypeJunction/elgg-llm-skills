...
**`tests/` vs `selftest/`** — `tests/` is the mirrored `elgg-migrate` engine suite
...
**`templates/elgg{N}/` vs `infra/elgg{N}/`** — they are different stacks and both
...
and re-run the generator — `bin/validate-elgg-infra.sh` fails if they drift.
