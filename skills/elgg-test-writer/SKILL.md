...
- **Namespaced constants in test bootstrap**: if a plugin lib file (`lib/functions.php`) uses a namespaced constant (e.g. `Acme\Geo\PLUGIN_ID`), define it in the test bootstrap BEFORE calling `\Elgg\Application::loadCore()`. Use the lowercase plugin ID value — 4.x normalizes all plugin IDs to lowercase. If the Bootstrap defines the constant in `load()` or `init()`, the test bootstrap must replicate it.
