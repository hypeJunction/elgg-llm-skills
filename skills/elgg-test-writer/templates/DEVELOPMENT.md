...
its PHPUnit, Playwright, and Vitest suites in isolation. The stack has no
dependency on any other plugin, workspace, or external tooling — cloning
this repo and running `docker compose up` is enough to get a working Elgg
site with only this plugin activated.
...
Open `http://localhost:${ELGG_PORT:-8480}` (Elgg 4) or `:8380` (Elgg 3)
for the running site. Default admin: `admin` / `admin12345`.
...
joins the same network as `elgg` and `db`, so tests talk to
`http://elgg` and `db:3306` directly.
...
- The only host path mounted into any container is **this plugin's source
  directory**. No sibling plugin is visible from inside Docker, so a
  destructive in-container command cannot touch anything outside this
  repo.
