...
# Write .env with PLUGIN_ID + a DETERMINISTIC per-plugin host-port pair (only if
# missing). The compose files default ELGG_PORT/DB_PORT to per-VERSION values, so
# two plugins on the same Elgg version would otherwise bind the same host port
# (and elgg6's DB default 3306 collides with a host-native MySQL). Deriving a
# stable offset from a hash of PLUGIN_ID gives each plugin its own high-range
# ports, reproducible across re-scaffolds; override in .env if two plugins collide.
port_offset=$(printf '%s' "$plugin_id" | cksum | cut -d' ' -f1)
port_offset=$((port_offset % 900))
elgg_port=$((20000 + port_offset))   # avoids 86xx per-version defaults and dev ports
db_port=$((21000 + port_offset))     # 21xxx — never the host-native MySQL 3306
...
if [ -f "$env_file" ] && [ "$force" -ne 1 ]; then
...
else
    {
        ...
        echo "PLUGIN_ID=$plugin_id"
        echo "# Deterministic per-plugin host ports (hash of PLUGIN_ID) so parallel"
        echo "# stacks on the same Elgg version don't collide. Override if needed."
        echo "ELGG_PORT=$elgg_port"
        echo "DB_PORT=$db_port"
    } > "$env_file"
    echo "  wrote: docker/.env (PLUGIN_ID=$plugin_id ELGG_PORT=$elgg_port DB_PORT=$db_port)"
fi
